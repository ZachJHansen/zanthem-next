...
pub fn simplify_formula(formula: Formula) -> Formula {
    ...
    loop {
        ...
        f2 = simplify_transitive_equality_deep(f2);
        f2 = simplify_empty_quantifiers_deep(simplify_variable_lists_deep(f2));

        f2 = restrict_quantifier_domain_deep(f2);
        f2 = simplify_empty_quantifiers_deep(simplify_variable_lists_deep(f2));
...
pub fn simplify_shallow(theory: Theory) -> Theory {
    Theory {
        formulas: theory
            .formulas
            .into_iter()
            .map(simplify_formula_shallow)
            .collect(),
    }
}

pub fn simplify_formula_shallow(formula: Formula) -> Formula {
...
pub fn join_nested_quantifiers_deep(formula: Formula) -> Formula {
    formula.apply(&mut join_nested_quantifiers)
}
...
pub fn extend_quantifier_scope_deep(formula: Formula) -> Formula {
    formula.apply(&mut extend_quantifier_scope)
}
...
pub fn simplify_variable_lists_deep(formula: Formula) -> Formula {
    formula.apply(&mut simplify_variable_lists)
}
...
pub fn simplify_empty_quantifiers_deep(formula: Formula) -> Formula {
    formula.apply(&mut simplify_empty_quantifiers)
}
...
pub fn restrict_quantifier_domain_deep(formula: Formula) -> Formula {
    formula.apply(&mut restrict_quantifier_domain)
}
...
pub fn eliminate_redundant_quantifiers_deep(formula: Formula) -> Formula {
    formula.apply(&mut eliminate_redundant_quantifiers)
}
...
pub fn simplify_transitive_equality_deep(formula: Formula) -> Formula {
    formula.apply(&mut simplify_transitive_equality)
}
...
mod tests {
    use {
        super::{
            ...
            simplify_empty_quantifiers, simplify_formula_shallow, simplify_transitive_equality,
    ...
    fn test_simplify() {
        for (src, target) in [
            ...
            assert_eq!(
                simplify_formula_shallow(src.parse().unwrap()),
                target.parse().unwrap()
            )
        }
    }
