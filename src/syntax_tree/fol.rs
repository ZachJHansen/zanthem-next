...
impl Formula {
    ...
    pub fn rename_variables(self, count: usize) -> (Formula, usize) {
        match self {
            Formula::QuantifiedFormula { quantification: q, formula: f } => {
                ...
                for (i, var) in q.variables.iter().enumerate() {
                    ...
                    let new_var = Variable { name: x, sort: var.clone().sort };
                    vars.push(new_var.clone());
                    let new_term = match var.sort {
                        Sort::General => {
                            GeneralTerm::GeneralVariable(new_var.name)
                        },
                        Sort::Integer => {
                            GeneralTerm::IntegerTerm(IntegerTerm::BasicIntegerTerm(BasicIntegerTerm::IntegerVariable(new_var.clone().name)))
                        },
                    };
                ...
                (Formula::QuantifiedFormula { 
                    quantification: Quantification { quantifier: q.quantifier, variables: vars }, formula: f1.into() }, fcount)
            },
            x => (x, count)
        }
    }
...
mod tests {
    ...
    fn test_rename_variables() {
        for (src, target) in [
            ...
            ("exists N M (exists N (N = M and p(N)) or q(N))", "exists X1 X2 (exists N (N = X2 and p(N)) or q(X1))"),
        ] {
